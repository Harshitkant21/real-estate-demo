...
import { apiCache, DEFAULT_CACHE_TTL_MS } from '../utils/apiCache';
...
/**
 * Fetch Live Properties for Sale from Property Finder API with Smart Cache Protection
 */
export const fetchLiveProperties = async (): Promise<NormalizedRecord<Property[]>> => {
  return apiCache.fetchWithCache('pf_live_properties', DEFAULT_CACHE_TTL_MS, async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (json && json.success && Array.isArray(json.data) && json.data.length > 0) {
          ...
          return {
            ...
            provenanceNotes: DATA_SOURCE_REGISTRY['pf-properties']?.licenseUsageNotes || 'Fetched from Property Finder API via RapidAPI (Smart Cache Active).',
          };
        }
      }
    } catch (err) {
...
/**
 * Fetch Live Off-Plan Launches from Property Finder API with Smart Cache Protection
 */
export const fetchLiveLaunches = async (): Promise<NormalizedRecord<Launch[]>> => {
  return apiCache.fetchWithCache('pf_live_launches', DEFAULT_CACHE_TTL_MS, async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (json && json.success && Array.isArray(json.data) && json.data.length > 0) {
          ...
          return {
            ...
            dataStatus: 'LIVE',
            lastVerifiedAt: new Date().toISOString(),
            provenanceNotes: DATA_SOURCE_REGISTRY['pf-launches']?.licenseUsageNotes || 'Fetched from Property Finder API via RapidAPI (Smart Cache Active).',
          };
        }
      }
    } catch (err) {
...
/**
 * Fetch verified real estate master developers from Property Finder API via RapidAPI with Smart Cache Protection
 */
export const fetchLiveDevelopers = async (): Promise<NormalizedRecord<Developer[]>> => {
  return apiCache.fetchWithCache('pf_live_developers', DEFAULT_CACHE_TTL_MS, async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (json && json.success && Array.isArray(json.data)) {
          ...
          return {
            ...
            dataStatus: 'LIVE',
            lastVerifiedAt: new Date().toISOString(),
            provenanceNotes: DATA_SOURCE_REGISTRY['pf-developers']?.licenseUsageNotes || 'Verified live stream from Property Finder RapidAPI (Smart Cache Active).',
          };
        }
      }
    } catch (err) {
...
/**
 * Fetch Live Market Metrics & Insights from Property Finder API with Smart Cache Protection
 */
export const fetchLiveMarketMetrics = async (): Promise<NormalizedRecord<MarketMetrics | null>> => {
  return apiCache.fetchWithCache('pf_live_market_metrics', DEFAULT_CACHE_TTL_MS, async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (json && json.success) {
          ...
          return {
            ...
            dataStatus: 'LIVE',
            lastVerifiedAt: new Date().toISOString(),
            provenanceNotes: 'Verified live market insights stream from Property Finder RapidAPI (Smart Cache Active).',
          };
        }
      }
    } catch (err) {
...
/**
 * Fetch Live FX Currency Rates from Open Exchange Rates API with Smart Cache Protection
 */
export const fetchLiveFXRates = async (): Promise<NormalizedRecord<CurrencyData | null>> => {
  return apiCache.fetchWithCache('open_fx_rates', 1000 * 60 * 60 * 12, async () => {
    try {
      ...
      if (fxData && fxData.rates) {
        return {
          ...
          dataStatus: 'LIVE',
          lastVerifiedAt: new Date().toISOString(),
          provenanceNotes: DATA_SOURCE_REGISTRY['open-fx']?.licenseUsageNotes || 'Fetched from Open Exchange Rates API (Cache Active).',
        };
      }
    } catch (err) {
