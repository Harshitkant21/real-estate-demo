...
const CURRENCY_DETAILS: Record<CurrencyCode, { name: string; nativeSymbol: string }> = {
  AED: { name: 'UAE Dirham', nativeSymbol: 'د.إ' },
  USD: { name: 'US Dollar', nativeSymbol: '$' },
  EUR: { name: 'Euro', nativeSymbol: '€' },
  GBP: { name: 'British Pound', nativeSymbol: '£' },
  INR: { name: 'Indian Rupee', nativeSymbol: '₹' },
  SAR: { name: 'Saudi Riyal', nativeSymbol: '﷼' },
};
  ...
  return (
    <div className="relative">
      <button
        ...
        className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold bg-stone-100/80 border border-stone-200 text-stone-800 rounded-lg hover:bg-stone-200 transition-colors"
      >
        ...
        <>
          ...
          <div className="absolute right-0 mt-2 w-64 bg-white border border-stone-200 rounded-xl shadow-xl z-50 p-2 space-y-1">
            <div className="flex items-center justify-between px-2 py-1.5 border-b border-stone-100 text-[11px] font-semibold text-stone-500">
              <span>Investor FX Currency</span>
              {onRefreshRates && (
                <button
            ...
            <div className="py-1">
              {APP_CONFIG.supportedCurrencies.map((code) => {
                ...
                const detail = CURRENCY_DETAILS[currencyCode];
                  ...
                  >
                    <div className="flex flex-col items-start">
                      <div className="flex items-center gap-1.5 font-bold">
                        <span>{code}</span>
                        <span className="text-amber-800 font-normal">({detail.nativeSymbol})</span>
                        {code === 'AED' && (
                          <span className="text-[9px] bg-amber-100 text-amber-900 px-1 py-0.2 rounded font-semibold">Base</span>
                        )}
                      </div>
                      <span className="text-[10px] text-stone-400 font-normal">{detail.name}</span>
                    </div>
